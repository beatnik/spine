...
{ my $proto = shift;
  ...
  $self->{ORDER} = "id";
  $self->{GROUP} = "id";
  $self->{NUMERIC} = [@SPINE::Base::Navbar::NUMERIC];
  $self->{NON_NUMERIC} = [@SPINE::Base::Navbar::NON_NUMERIC];
  $self->{FIELDS} = [qw(id name alignment positioning font color size style sep modified owner usergroup permissions )];
  bless $self,$class;
  return $self;
}
