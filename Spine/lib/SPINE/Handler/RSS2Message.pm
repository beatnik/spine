package SPINE::Handler::RSS2Message;
...
{ my $tr_req = shift; #SPINE::Transparent::Request ; Apache::Request
  ...
  { next if ref($message) ne "SPINE::Base::Message";
    ...
    my $msgbody = $message->body;
    $msgbody =~ s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
    $mbody =~ s/<\?SPINE_Message_Body\?>/$msgbody/gmxe;
...
SPINE::Handler::RSS2Message

=head1 DESCRIPTION

This is the RSS2 Message Handler for SPINE.
