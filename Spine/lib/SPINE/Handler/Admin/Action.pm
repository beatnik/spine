package SPINE::Handler::Admin::Action;
...
use vars qw($VERSION $content_dbi $user_dbi $usergroup_dbi $session_dbi $user $adminaccess_dbi $session_dbi $macro_dbi $request $user $adminaccess $adminaccess_dbi $action_dbi $request $error $readperms $writeperms $execperms $attribute_dbi %i18n %default);
...
{ $request = shift; #SPINE::Transparent::Request ; Apache::Request
  ...
  { $action_dbi->add(SPINE::Base::Action->new({section=>"mimetype", name=>"content", attr=>$request->param('attr'), value=>$request->param('value')}));
  }
  ...
  my $list = undef;
  $list .= qq(<div name="adminpanel" class="fullpanel"><form action="<?SPINE_Location?>admin/action/new/" method="post">\n);
...
SPINE::Handler::Admin::Action

=head1 DESCRIPTION

This is the Timed Action Admin Handler for SPINE.
