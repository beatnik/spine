...
{ my $request = shift; #SPINE::Transparent::Request ; Apache::Request
  ...
  if ($action eq "upload" && $filename && $request->method() eq "POST")
  ...
  $list = <<"EOF";
<form name="uploadform" method="post" action="<?SPINE_Location?>admin/file" enctype="multipart/form-data">&nbsp;&nbsp;Upload as content : <input type="hidden" name="action" value="upload"><input type="file" name="filename">&nbsp;<input type="submit" class="button" value="Upload"></form>
$error
EOF
