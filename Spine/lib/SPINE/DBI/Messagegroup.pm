package SPINE::DBI::Messagegroup;
...
{ my $proto = shift;
  ...
  $self->{TABLE} = "messagegroup";
  $self->{MODULE} = "Messagegroup";
  $self->{ORDER} = 'id, name';
  $self->{GROUP} = 'name';
  $self->{NUMERIC} = [@SPINE::Base::Messagegroup::NUMERIC];
  $self->{NON_NUMERIC} = [@SPINE::Base::Messagegroup::NON_NUMERIC];
  $self->{FIELDS} = [qw(id name owner usergroup permissions content)];
  bless $self,$class;
  return $self;
}
