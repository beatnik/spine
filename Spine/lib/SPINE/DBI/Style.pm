...
{ my $proto = shift;
  ...
  $self->{ORDER} = 'name';
  $self->{GROUP} = "name";
  $self->{NUMERIC} = [@SPINE::Base::Style::NUMERIC];
  $self->{NON_NUMERIC} = [@SPINE::Base::Style::NON_NUMERIC];
  $self->{FIELDS} = [qw(id name title macros modified owner usergroup permissions body icomment)];
  bless $self,$class;
  return $self;
}
...
This is the Style DBI class for SPINE.
