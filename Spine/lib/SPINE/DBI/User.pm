...
{ my $proto = shift;
  ...
  $self->{NUMERIC} = [@SPINE::Base::User::NUMERIC];
  $self->{NON_NUMERIC} = [@SPINE::Base::User::NON_NUMERIC];
  $self->{FIELDS} = [qw(id usergroup login password fullname email)];
  bless $self,$class;
  return $self;
}
