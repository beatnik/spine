...
{ my $proto = shift;
  ...
  $self->{ORDER} = "name";
  $self->{GROUP} = "name";
  $self->{NUMERIC} = [@SPINE::Base::Wiki::NUMERIC];
  $self->{NON_NUMERIC} = [@SPINE::Base::Wiki::NON_NUMERIC];
  $self->{FIELDS} = [qw(id name modified owner usergroup permissions body)];
  bless $self,$class;
  return $self;
}
...
This is the Wiki DBI class for SPINE.
