...
{ my $proto = shift;
  ...
  $self->{TABLE} = "navbarbuttons";
  $self->{MODULE} = "Button";
  $self->{ORDER} = "position";
  $self->{GROUP} = "navbar";
  $self->{NUMERIC} = [@SPINE::Base::Button::NUMERIC];
  $self->{NON_NUMERIC} = [@SPINE::Base::Button::NON_NUMERIC];
  $self->{FIELDS} = [qw(id navbar position link image label target border width height font color size style class) ];
  bless $self,$class;
  return $self;
}
