...
{ my $proto = shift;
  ...
  $self->{MODULE} = "Usergroup";
  $self->{ORDER} = 'usergroup';
  $self->{NUMERIC} = [@SPINE::Base::Usergroup::NUMERIC];
  $self->{NON_NUMERIC} = [@SPINE::Base::Usergroup::NON_NUMERIC];
  $self->{FIELDS} = [qw(id usergroup username)];
  bless $self,$class;
  return $self;
}
...
This is the Usergroup DBI class for SPINE.
