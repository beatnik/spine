...
{ my $proto = shift;
  ...
  $self->{TABLE} = "adminaccess";
  $self->{MODULE} = "Adminaccess";
  $self->{ORDER} = "section";
  $self->{GROUP} = "usergroup";
  $self->{NUMERIC} = [@SPINE::Base::Adminaccess::NUMERIC];
  $self->{NON_NUMERIC} = [@SPINE::Base::Adminaccess::NON_NUMERIC];
  $self->{FIELDS} = [qw(id section usergroup permissions )];
  bless $self,$class;
  return $self;
}
...
SPINE::DBI::Adminaccess

=head1 DESCRIPTION

This is the User DBI class for SPINE.
