package SPINE::Handler::PicasaGallery;
...
{ my $request = shift; #SPINE::Transparent::Request ; Apache::Request
  ...
  if ($dir && !$pic)
  { opendir(DIRECTORY,"$images/images/$dir/thumbnails") || die "$images/images/$dir : $!";
    ...
    { if ($_ =~ /^\./mx) { next; }
      ...
      $foo++;	
	  if ($foo == 7) { $body .= "</tr><tr>\n"; $foo = 2; }
      if ($_ =~ /.*\.png$/mxi || $_ =~ /.*\.jpe?g$/imx || $_ =~ /.*\.gif$/imx) 
      { my $f = $_; $f = uri_escape($_); $body .= qq(<td align="center"><a href="$url?dir=$dir&pic=$f">\n<img src="$location/images/$dir/thumbnails/$_">\n<br>$_</a></td>); next; }
    }
    $body .= "</tr></table>";
    closedir(DIRECTORY);
  }
  if ($dir && $pic)
  { opendir(DIRECTORY,"$images/images/$dir/thumbnails") || die $!;
    my @files = readdir(DIRECTORY);
    shift @files; shift @files;
    ...
    $body .= qq(<img src="$location/images/$dir/images/$pic"><br><form method="get" action="$url"><input type="hidden" name="dir" value="$dir"><input type="hidden" name="pic" value="$pic">Your name: <input type="text" name="name"><br>Comments:<br><textarea rows="10" cols="40" name="comment"></textarea><br><input type="submit" value="Done"></form><br><?SPINE_Message("$dir","","$pic")?>\n); }
  return $body;
}
