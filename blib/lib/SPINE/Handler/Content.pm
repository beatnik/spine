...
{ my $request = shift; #Apache::Request
  ...
  { $content = shift @{$content_dbi->get({name=>$url, count=>1})};
    ...
    if ($session && $session->host eq scalar($request->connection->get_remote_host()) && $session->username)
    ...
    if ($session && $user && $session->username eq $user->login && $session->host eq scalar($request->connection->get_remote_host()) && !$loadpage)
